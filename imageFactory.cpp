...
ImageFactory& ImageFactory::getInstance() {
  static ImageFactory instance;//myersss????
  return instance;
...
ImageFactory::~ImageFactory() {
  ...
  std::map<std::string, SDL_Surface*>::const_iterator itr = surfaces.begin();
  ...
  std::map<std::string, SDL_Texture*>::const_iterator itrTexture = textures.begin();
  while(itrTexture != textures.end()){
    SDL_DestroyTexture(itrTexture->second);
    itrTexture++;
  }
  ...
  std::map<std::string, Image*>::const_iterator i = images.begin(); 
  
  while(i != images.end()){
    std::cout << "deleting" << i->first << std::endl;
    delete i->second;
    i++;
  }
